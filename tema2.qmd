...
::: {.callout-tip title="Código Python: Búsqueda Dicotómica" collapse="true"}
```python
...
for k in range(1, 10):
    ...
    f_l = f(lmbda)
    f_m = f(mu)
    print(f"{k:9d} | {a:7.4f} | {b:7.4f} | {c:7.4f} | {lmbda:8.4f} | {mu:7.4f} | {f_l:9.4f} | {f_m:8.4f}")
    if f_l < f_m:
        b = mu
    else:
        a = lmbda
...
::: {.callout-tip title="Código Python: Sección Áurea" collapse="true"}
```python
import math
f = lambda x: 0.25*x**4 - (13/3)*x**3 + 27*x**2 - 72*x + 1
a, b = 1.0, 4.0
...
print("Iteración | a_k     | b_k     | lambda_k | mu_k    | f(lambda) | f(mu)")
print("-" * 75)
...
::: {.callout-tip title="Código Python: Búsqueda de Fibonacci" collapse="true"}
```python
f = lambda x: 0.25*x**4 - (13/3)*x**3 + 27*x**2 - 72*x + 1
a, b = 1.0, 4.0
n = 6
...
::: {.callout-tip title="Código Python: Método de Bisección" collapse="true"}
```python
...
print("Iteración | x_k     | f'(x_k)  | f''(x_k) | x_k+1")
print("-" * 55)
for k in range(1, 10):
    val_df = df(x)
    val_ddf = ddf(x)
...
::: {.callout-tip title="Código Python: Máximo Descenso" collapse="true"}
```python
import numpy as np
